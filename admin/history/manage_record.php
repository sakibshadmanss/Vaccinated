...
<div class="card card-outline card-info">
	<div class="card-header">
		<h3 class="card-title"><?php echo isset($id) ? "Track and Update ": "Track and Update " ?> Record</h3>
	</div>
	<div class="card-body">
			...
			<fieldset id="prev_form" class="">
				...
				<div id="indiviual_info" class="row">
					...
					<dl>
                        ...
                        <dt class="text-muted">Vaccine:</dt>
                        <dd class="pl-4 indi_Vaccine_type"><?php echo isset($result_indi['Vaccine_type']) ? $result_indi['Vaccine_type'] : '' ?></dd>
					...
					<div class="col-md-6">
						<dl>
							...
							<dt class="text-muted">Vaccine Status:</dt>
			...
			<div class="form-group row">
				<div class="col-md-6">
					...
					<select name="vaccine_id" id="vaccine_id" class="custom-select custom-select-sm select2" required>
						...
						?>
						<option value="<?php echo $row['id'] ?>" <?php echo isset($vaccine_id) && $vaccine_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['name'].' | '.$row['Category']; ?></option>
						<?php endwhile; ?>
					</select>
				</div>
			</div>
				...
				<div class="col-md-6">
					<div class="mb-2">
						...
						<select name="vaccination_type" id="vaccination_type" class="custom-select selevt" required>
							...
							<option value="6th Dose">6th Dose</option>
			...
			<div class="form-group row">
				<div class="col-md-6">
					...
					<select name="status" id="status" class="custom-select selevt" required>
						<option value="0" <?php echo isset($status) && $status == 0 ? 'selected' : '' ?>>Not Vaccinated</option>
						<option value="1" <?php echo isset($status) && $status >= 1 ? 'selected' : '' ?>>Partially Vaccinated</option>
						<option value="999" <?php echo isset($status) && $status == 999 ? 'selected' : '' ?>>Fully Vaccinated</option>
					</select>
				</div>
			</div>
			<?php endif; ?>
			...
			<div class="form-group row">
				<div class="col-md-6">
					...
					<select name="location_id" id="location_id" class="custom-select custom-select-sm select2" required>
						...
						<?php 
							$location_qry = $conn->query("SELECT * FROM hospital");
							while($row = $location_qry->fetch_assoc()):
						?>
						<option value="<?php echo $row['id'] ?>" <?php echo isset($location_id) && $location_id == $row['id'] ? 'selected' : '' ?>><?php echo $row['hospitalName'] ?></option>
						<?php endwhile; ?>
					</select>
				</div>
			</div>
			...
			$.ajax({
				...
				success:function(resp){
					if(!!resp.id){
						...
						$('dd.indi_Vaccine_type').text(resp.Vaccine_type)
