			...
			<table class="table table-bordered table-striped">
				...
				<tbody>
					...
					$i = 1;
						...
						$qry = $conn->query("SELECT h.*,concat(i.firstname,' ',i.middlename,' ', i.lastname) as iname,i.tracking_code FROM `vaccine_history_list` h inner join individual_list i on h.individual_id = i.id {$where} order by unix_timestamp(h.date_created) desc");
	...
	function delete_history($id){
		...
		$.ajax({
			...
			success:function(resp){
				if(typeof resp== 'object' and resp.status == 'success'){
					location.reload();
				}else{
					alert_toast("An error occured.",'error');
					end_loader();
